function [logjointProb] = JointTargetDensity_OWFYearlyRound_TrnNrm_NN(smpPar, samples, FarmData)
...
% Hyper-prior samples
sample(1,1) = log(pdf('Uniform', smpMu(1), 0, 100)) + log(pdf('Uniform', smpMu(2), 0, 40)) + sum(log(pdf('Uniform', smpTau, 0, 30)));
% sample(1,1) = 0; 
...
for i = 1:length(unique(FarmData.Round))
    ...
    if or(lt(smpThetaRound(2+i),0), lt(smpTau(4),0))
        ...
        relProbParVar(indRound) = log(TruncNorm(smpThetaStd(indRound), 0, 40, smpThetaStd(i), smpTau(4)^2, [], 'PDF'));
